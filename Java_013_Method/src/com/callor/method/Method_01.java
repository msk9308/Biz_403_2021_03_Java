...
//		= new Random();
		 ...
		 *조건에 완전히 일치하는 경우에는 
		 *~~ 하다. ~~다 라고표현하고 
		 *조건에 일치하지않는 else를 사용하여 표현할때는
		 *딱 맞아 떨어지는 어떤 조건을 찾기가 어렵기때문에 
		 *~~가 아니다 라고 표현하는것이 좋다
		 */
		int int1 = rnd.nextInt(100)+1;
		if(int1 % 3 == 0) {
			System.out.println(int1+"은 3의배수");
		}else {
			System.out.println(int1+"은3의배수가 아니다");
		}
		/*
		 * 어떤수가 짝수인가를 조건검사하고
		 * 짝수이면 짝수이다 라고 표현하고
		 * else이면 홀수이다 라고 표현할수 있지만
