...
public class ScoreServiceImplV2A extends ScoreServiceImplV2 {
	...
	public void inputScore() {
		...
		//subject배열을 멤버영역으로 이동 subjectList로
		//점수를 입력하는 곳에서 사용하기 위하여
		//과목명을 담은 배열을 선언했는데
		//생각해보니 다른 method에서도 ㄷ\사용할수 있을것 같다
		//그래서 클래스 영역으로 이동하였다
		
		
		//과목수만큼 점수를 저장할 배열을 선언
		//Integer[] intScore = new Integer[subject.length];
		//intScore배열을 멤버영역으로 이동 subScoreList로
		Integer intKor = inService.inputValue("국어",0,100);
		
		for(int i = 0; i <subjectList.length; i++) {
			subsScoreList[i] = inService.inputValue(subjectList[i],0,100);
			if(subsScoreList[i] == null) {
				return;
			}
		}
		...
		scoreVO.setKor(subsScoreList[국어]);
		scoreVO.setEng(subsScoreList[영어]);
		scoreVO.setMath(subsScoreList[수학]);
		
		scoreVO.setKor(subsScoreList[Values.국어]);
		scoreVO.setEng(subsScoreList[Values.영어]);
		scoreVO.setMath(subsScoreList[Values.수학]);
