 ...
 * 			List클래스를 사용한 객체를 생성하여 진행을 한다
 * 
 */

public class IolistServiceV1 {
	/*
	 * 매입 매출 데이터들을 사용하여 연산을 수행할텐데 매입 매출 데이터들의 묶음을 List형 객체 사용하겠다
	 * 
	 * List형 객체에 포함된 (개별)데이터는 IolistVO형으로 사용하겠다
	 * 
	 * 낱개 데이터는 IolistVO 클래스형으로 사용하고 그 데이터들을 묶어서 List형으로 연산 수행을 하겠다라는 선언
	 * 
	 * iolist를 private로 선언한이유는 >> 혹시 외부에서 iolist데이터에 접근하여 값을 저장하는 일을 방지하기 위함이다 그러나
	...
	// public 으로 선언된 method는 객체.method() 형식으로
	// 호출이 가능한 method
