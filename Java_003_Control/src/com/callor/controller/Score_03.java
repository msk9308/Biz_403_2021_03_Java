...
public class Score_03 {
	...
	public static void main(String[] args) {
		...
		/*
		 * 과목의 평균을 계산할 때 소수점 이하 오류가 발생하여
		 * 소수점 이하 계산을 살리기 위해
		 * 평균을 저장할 변수를 float 으로 선언하였다
		 * 하지만, 여전히 계산된 결과는 소수점이하 값이 0으로 
		 * 나오고 있다
		 * 
		 * 이상황은 대입연산자의 성질때문이다 
		 * 대입연산자가 나오면 항상 오른쪽의 연산식이 먼저
		 * 실행된다
		 * 결과적으로 정수 / 정수는 결과가 정수이며
		 * 이미 계산결과에서 소수점이하 값이 없는상태이다
		 * 이 값을 실수형변수에 담는것은 아무런 의미가 없다
		 * 
		 * 이 명령문을 실행하여 정확한 평균을 계산하기 위해서는
		 * 대입연산자 오른쪽의 계산식의 값들을 실수형으로 
		 * 강제 형변환 한 다음 연산을 수행해야 한다
		 */
		//평균을 저장할 변수를 float형으로 
		float floatAvg = (float)intSum / 2;
