 ...
 * 한개의 모듈(파일)에 또는 한 프로젝틍서 같은 이름의 함수를
 * 선언할 수 없다 
 * 
...
public class NumberServiceV1 {
	 ...
	 * 			객체.addNum(100, 200)
	 * 두개의 정수를 전달받기 위해서는 method가 준비를 해야 한다
	 * 		method의 ()안에 변수를 선언한다
	 * 		두개의 변수를 선언하자 
	 * 
	 * 두 수의 합을 계산하여 
	 ...
	 * 
	 * 		단순히 정수만 return하는 경우는 int type으로 하면 되는데
	 * 		여기에서는 어떤 조건에 따라 
	...
	public Integer addNum(int num1, int num2) {
		...
		//		return문은 한번만 쓰자 라는 전통적인 패턴이 있다
		//		전통적인 패턴을 지키는 코드 예제
//		Integer sum = null;
//		if((num1 + num2) == 0) {
//			sum = num1 + num2;
...
//		return sum;
	 ...
	 * 		addNum(long num1, long num2),(int num1, int num2)형식으로 method를 선언해야한다
	 * 
	 * 만약 addNum(실수,실수)방식으로 호출하려면
	 * 		addNum(float num1, float num2),(double num1,double num2)형식으로 method를 선언해야 한다
	 * 
