...
public class ScoreServiceV4 {
	...
	public void makeScores() {
		...
		for(int i = 0; i < intKor.length; i++) {
			...
			for (int j = 0; j < subject.length; j++) {
				...
				/*
				 * 입력 업무중에 -1을 입력했을대는
				 * 입력업무를 중단하고 main()으로 되돌아가고싶다
				 * 이때 break를 사용하면 for() 겹쳐있기 때문에원하는대로 
				 * 코드가 작동되지 않는다
				 * 이럴경우 return type 이 void 이지만
	...
	private Integer inputScore(String sub) {
		...
		while(true) {
			...
			System.out.println(">>");
			score = scan.nextInt();
			//점수입력을 하는중에 -1을 입력하면 입력하는 업무를 중단하자
			if(score == -1) {
				return -1;
